...
public class Vector2D implements Comparable<Vector2D>{
    ...
    @Override
    public int compareTo(@NotNull Vector2D that) {
        if ((this.x == that.x) || (Double.isNaN(this.x) && Double.isNaN(that.x))) {
            if (this.y == that.y) {
                return 0;
            }
    ...
    public int minYOrderedCompareTo(Vector2D that){
    ...
    public static Vector2D midpoint(Vector2D a, Vector2D b){
        return a.getAdded(b).getMultiplied(0.5);
    }
